...
public class EstesTruckloadRegressionAPITest extends TestBase {
	...
	public String generateQuoteNumber() throws Exception {
		...
		// Step-3 & 4 : Body Note: pickupDate should be 2 days in the future (not a
		// weekend or a holiday)and click send
		Response response = given().auth().basic(userName, password).header("Content-Type", "application/json")
				...
				.body("{\r\n" + "    \"requestedBy\": \"TBP-670\",\r\n" + "    \"account\": \"4137007\",\r\n"
						...
						+ "    \"accessorialList\": [\r\n" + "        {\r\n"
						+ "            \"accessorialCode\": \"\"\r\n" + "        }\r\n" + "    ]\r\n" + "}")
				.when().post(baseURI).then().log().all().statusCode(200).statusLine("HTTP/1.1 200 OK").extract()
				.response();
	 ...
	 * @author Nithya
	 * 
	 * Verify when an Estes Truckload quote is successfully booked and the user
	 * re-select the API Send button then a message is displayed informing the user
	 * that the quote has already been booked & to contact the Truckload team for
	 * assistance
	 */
	

	@Test(enabled = false, priority = 1)
	public void executeQZ_10343()
			throws ParseException, InterruptedException, Exception {
		...
		// Step-9 and Step-10 : replace quoteId by the quote id noted in step 5 and
		// Click Send
		Response response = given().auth().basic(userName, password).header("Content-Type", "application/json")
				.contentType(ContentType.JSON).accept(ContentType.JSON)
				.body("{\r\n" + "    \"quoteId\": \"" + quoteNum + "\",\r\n"
						...
						+ "    \"temperatureLow\": \"\",\r\n" + "    \"temperatureHigh\": \"\"\r\n" + "}")
				.when().post(pickupURI).then().log().all().statusCode(200).statusLine("HTTP/1.1 200 OK").extract()
				.response();
		...
		Response response2 = given().auth().basic(userName, password).header("Content-Type", "application/json")
				.contentType(ContentType.JSON).accept(ContentType.JSON)
				.body("{\r\n" + "    \"quoteId\": \"" + quoteNum + "\",\r\n"
						+ "    \"contactName\": \"Requester Contact Name\",\r\n"
						+ "    \"contactCompany\": \"ESTES to CHR\",\r\n"
	 ...
	 * @author Nithaya
	 * 
	 * Verify that when API user keyed in origin or destination address is AK then
	 * an error message ' The origin address provided is not in the contiguous
	 * United States. Please update your request and submit again. For shipments xxx
	 * ' is returned
	 */

	@Test(enabled = true, priority = 2)
	public void executeQZ_10329()
			throws ParseException, InterruptedException, Exception {
	...
	public void executeQZ_9336() {
		...
		List<WebElement> eiaPrices = driver.findElements(
				By.xpath("/html/body/div[1]/div[2]/div/div[4]/div[1]/div/div[3]/table/tbody/tr[*]/td[4]"));
